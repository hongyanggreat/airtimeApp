...
//    private static final HashMap<String, ChargingParams> ReqCacheManager = new HashMap<>();
    static final Logger logger = Logger.getLogger(OtpRequest.class);
        ...
        if (chParam != null) {
            ...
            try {
                ...
                if (acc == null) {
                    ...
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.AUTHEN_FAIL.getJson());
                ...
                if (!cpCode.equals(chParam.getCpCode())) {
                    ...
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.CP_CODE_FAIL.getJson());
...
//                }
                if (!validIP) {
                    ...
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.IP_NOT_ALLOW.getJson());
                ...
                if (!PhoneValid.validPhoneViettel84(chParam.getMsisdn())) {
                    ...
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.PHONE_NOT_VALID.getJson());
                    return ERR_CODE.PHONE_NOT_VALID.getJson();
                    // Valid phone 
                }
                if (Tool.checkNull(chParam.getPrice())) {
                    System.out.println("GHI LOG FILE : TH getMsisdn null");
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.PRICE_NOT_NULL.getJson());
                ...
                if (NumberUtils.toInt(chParam.getPrice()) <= 0) {
                    ...
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.PRICE_NOT_VALID.getJson());
...
//                TAO CACHE REQUEST OTP
                ...
                if (ReqCacheManager.get(keycache) != null) {
                    ...
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.REQUEST_EXIST.getJson());
...
//                    CHECK REQUEST LAP LAI NHIEU LAN
                ...
                if (totalReqOtp >= 5) {
                    ...
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.REPEAT_OTP.getJson());
...
//                    SAU NAY CHECK PRICE HOP LE THI MOI LAY SUBSERVICE
                if (Tool.checkNull(subService)) {
                    ...
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.UNKNOW_SERVICE.getJson());
...
//                    CHECK SO TIEN /LAN <= 100k
                ...
                if (!checkPrice) {
                    ...
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.AMOUNT_LIMIT.getJson());
...
//                    CHECK SO DIEN THOAI NAP QUA 500k
                ...
                if (!checkTotalAmount) {
                    ...
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.TOTAL_AMOUNT_LIMIT.getJson());
...
//                    LUU DB O DAY TRUOC KHI RETURN
                if (result_mps.equals("-1")) {
                    ...
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.DISCONNECT_MPS.getJson());
...
//                    Check KET QUA 
                if (result_mps.equals("4")) {
                    ...
                    System.out.println("-------------------");
                    doAddRequestOtp(request, chParam, result_mps, ERR_CODE.REQUIRE_OTP.getJson());
                    ...
                    doAddRequestOtp(request, chParam, result_mps, mapErrorMps);
                    return mapErrorMps;
                }
...
//        return result;
    ...
    public static boolean doAddRequestOtp(String request, ChargingParams chParam, String rs_mps, String rs_ahp) {
        boolean rs = false;
        ...
        rs = airTimeRequest.addRequestOtp(airTimeRequest);
